...
printf '%s' "$template_content" > "$ROOT_DIR/$rendered_task_def"

log "register task definition"
TASK_DEF_ARN="$(aws ecs register-task-definition --cli-input-json "file://$ROOT_DIR/$rendered_task_def" --query taskDefinition.taskDefinitionArn --output text --region "$REGION")"
